...
export function ProjectCronDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-3xl overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-950 shadow-2xl">
        {/* Header */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                Cron Jobs
                <span className="rounded-md border border-neutral-800 bg-neutral-900 px-1.5 py-0.5 font-mono text-[11px] text-neutral-300">{projectKey}</span>
              </h2>
              <p className="text-xs text-neutral-400">Scheduled automation for {projectName}.</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {canManage && (
              <button
                ...
                className="cursor-pointer flex items-center gap-1.5 rounded-xl bg-white px-3 py-1.5 text-xs font-bold text-black hover:bg-neutral-200 transition"
              >
        ...
        <div className="max-h-[75vh] overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {loading ? (
              <div className="flex h-24 items-center justify-center rounded-2xl border border-neutral-800 bg-neutral-950">
                <Loader2 className="h-5 w-5 animate-spin text-neutral-500" />
              </div>
            ) : jobs.length === 0 ? (
              <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-6 text-center text-neutral-500 text-xs">
                <Bot className="h-6 w-6 mx-auto text-neutral-600 mb-1" />
              ...
              jobs.map((job) => {
                ...
                return (
                  <div key={job.id} className="rounded-2xl border border-neutral-800 bg-neutral-950 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-white text-sm truncate">{job.name}</span>
                          <span
                        ...
                        {job.description && (
                          <p className="text-[11px] text-neutral-500 mt-1 line-clamp-2">{job.description}</p>
                        )}
                      </div>
                      {canManage && (
                        <button
                    ...
                    <div className="mt-2.5 flex flex-wrap gap-3 text-[11px] text-neutral-400 pt-2 border-t border-neutral-900">
                      <span className="flex items-center gap-1"><Calendar className="h-3 w-3 text-emerald-400" /> Next: <strong className="text-neutral-200">{fmtDate(job.nextRunAt)}</strong></span>
          ...
          <div className="space-y-2">
            ...
            {loading ? (
              <div className="flex h-20 items-center justify-center rounded-2xl border border-neutral-800 bg-neutral-950">
                <Loader2 className="h-5 w-5 animate-spin text-neutral-500" />
              </div>
            ) : logs.length === 0 ? (
              <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-6 text-center text-neutral-500 text-xs">
              ...
              <div className="space-y-2">
                {logs.map((log) => (
                  <button
                    ...
                    className="cursor-pointer w-full flex items-center justify-between gap-3 rounded-2xl border border-neutral-800 bg-neutral-950 p-3.5 text-left hover:bg-neutral-900/60 transition"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs font-bold text-white truncate">{log.cronJobName || "Manual Run"}</div>
                        <p className="text-[11px] text-neutral-400 truncate">{log.summary}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0 text-[11px] text-neutral-500">
